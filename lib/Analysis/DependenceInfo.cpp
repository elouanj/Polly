...
#define MAXLNO 16384
#define LIDSIZE 14
llvm::cl::opt<std::string> FileMappingPath("fm-path", llvm::cl::init(""),
	llvm::cl::desc("Specify file mapping location"), llvm::cl::Hidden);
llvm::Type *pointsToStruct(llvm::PointerType *PTy);

std::map<std::string, llvm::Value*> VarNames;
std::map<std::string, llvm::MDNode*> Structs;
int32_t fileID;
int instructionNumberID = 0;
...
std::vector<instructionStruct> insts;

//#include "discopop/CUGeneration/CUGenerationPass.cpp"
//#include "discopop/share/include/DPUtils.h"
...
inline std::string decodeLID(int32_t lid)
{
  if (lid == 0)
    return "*";
...
int32_t getFileID(std::string fileMapping, std::string fullPathName) {
	int32_t index = 0; // if the associated file id is not found, then we return 0
	std::string line;
	std::ifstream fileMap(fileMapping.c_str());

	if (fileMap.is_open()) {
		std::vector<std::string>* substrings = NULL;
		while (getline(fileMap, line)) {
			substrings = split(line, '\t');
			if (substrings->size() == 2) {
				std::string indexString = (*substrings)[0];
				std::string fileName = (*substrings)[1];
...
{
  //LLVM_DEBUG(dbgs() << "\nInstruction getLID" << *BI << "\n";);
    int32_t lid = 0;
    int32_t lno;

    const llvm::DebugLoc &location = BI->getDebugLoc();
    if (location)
    {
...
/*int32_t determineFileID(Function &F, int32_t& fileID)
{
    fileID = 0;
                ...
                if (lno)
                {
                    ...
                    if (N)
                    {
                        ...
                        if (absolutePathFileName)
                        {
                            ...
                            fileID = getFileID(FileMappingPath, std::string(absolutePathFileName));
                            delete[] absolutePathFileName;
                        }
                        LLVM_DEBUG(dbgs() << "absolutePathFileName " << absolutePathFileName << "\n";);
...
std::string getOrInsertVarName(std::string varName, llvm::IRBuilder<>& builder) {
    Value* valName = NULL;
    std::string vName = varName;
    std::map<std::string, Value*>::iterator pair = VarNames.find(varName);
...
std::string findStructMemberName(MDNode* structNode, unsigned idx, llvm::IRBuilder<>& builder) {
    assert(structNode);
...
//=defaultIsGlobalVariableValue
std::string determineVariableName(Instruction *I, bool &isGlobalVariable )
{
    ...
    if (operand->hasName())
    {
        ...
        if (isa<GlobalVariable>(*operand))
        {
            ...
            return std::string(operand->getName().str());
        }
        if (isa<GetElementPtrInst>(*operand))
        {
            GetElementPtrInst *gep = cast<GetElementPtrInst>(operand);
            Value *ptrOperand = gep->getPointerOperand();
            llvm::PointerType *PTy = cast<PointerType>(ptrOperand->getType());
            ...
            if (structType && gep->getNumOperands() > 2)
            {
                ...
                if (constValue && isa<ConstantInt>(*constValue))
                {
                    ...
                    {
                        std::string strName(structType->getStructName().data());
                        std::map<std::string, MDNode *>::iterator it = Structs.find(strName);
                        if (it != Structs.end())
                        {
        ...
        return std::string(operand->getName().data());
...
//start add
void printInstructionsDiscoPoP(raw_ostream &OS) {
  OS << "\n";
  for(instructionStruct instruction : insts){
    OS << "STATEMENT NAME : " << instruction.StmtName << "\n";
    OS << "TEXT : " << *instruction.Text << "\n";
    OS << "VAR NAME : " << instruction.VarName << "\n";
    OS << "LINE ID : " << instruction.LineID << "\n";
    OS << "LINE NUMBER : " << instruction.LineNumber << "\n";
    OS << "\n";
  }
}

void Dependences::finalPrint(int kind, std::string type, raw_ostream &OS){
  isl::union_map results = getDependences(kind);
  auto mapList = results.get_map_list();

  for (isl::map islmapping : mapList) {
    //OS << "\nmap : " << islmapping.get();

    auto domainId = islmapping.get_domain_tuple_id();
    std::string srcStmt = domainId.name();

    auto rangeId = islmapping.get_range_tuple_id();
    std::string destStmt = rangeId.name();

    OS << "\nsrcStmt : " << srcStmt << " destStmt : " << destStmt;
    std::string srcLineNumber;
    std::string destLineNumber;
    std::string VariableName;
    bool srcCheck = false;
    bool destCheck = false;

    for(instructionStruct instruction : insts){
      if(instruction.StmtName == srcStmt){
        srcLineNumber = instruction.LineNumber;
        if(type == "WAR" || type == "WAW")
          VariableName = instruction.VarName;
        srcCheck = true;
      }

      if(instruction.StmtName == destStmt){
        destLineNumber = instruction.LineNumber;
        if(type == "RAW")
          VariableName = instruction.VarName;
        destCheck = true;
      }
    ...
    OS << "\n" << type << " src : " << srcLineNumber << " dest : " << destLineNumber << " var : " << VariableName;
  }
}

void Dependences::printDependenciesDiscoPoP(raw_ostream &OS, Scop &S) {
  ...
  OS << "\nNEW PRINT OF DEPENDENCIES :\n";
  llvm::SmallString<128> FilenameVec;

  for (ScopStmt &Stmt : S) {
    auto stmtName = Stmt.getBaseName();
    FilenameVec = StringRef(Stmt.getEntryBlock()->getModule()->getName());
    llvm::sys::fs::make_absolute(FilenameVec);
    OS << "SOURCE FILE PATH : " << FilenameVec << "\n";

    fileID = getFileID(FileMappingPath, std::string(FilenameVec));
    OS << "FILE ID : " << fileID << "\n";

    for (Instruction *Inst : Stmt.getInstructions()){
      auto varname = determineVariableName(Inst, isGlobalVariable);
      //determineFileID(*F, fileID);
      auto lid = getLID(Inst, fileID);
      auto lnu = decodeLID(lid);
      auto text = Inst;
      if(std::string(Inst->getOpcodeName()) == "store"){
        inst.LineID     = lid;
        inst.Text       = text;
        inst.VarName    = varname;
        inst.LineNumber = lnu;
        inst.StmtName   = stmtName;
        insts.push_back(inst);
        break;
      }
    }
  }
  printInstructionsDiscoPoP(OS);
  ...
  finalPrint(TYPE_RAW, "RAW", OS);
  inalPrint(TYPE_WAW, "WAW", OS);
  finalPrint(TYPE_WAR, "WAR", OS);
...
void polly::DependenceInfo::printScop(raw_ostream &OS, Scop &S) const {
  ...
  D.calculateDependences(S);
  D.print(OS);
  D.printDependenciesDiscoPoP(OS, S);
}
